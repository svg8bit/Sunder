...
- Persist browser-wallet connector choice and reconnect safely across routes/reloads. Solana also supports user-requested embedded wallets: generate them only in the secure browser, encrypt the 64-byte secret immediately with a non-extractable AES-GCM device key in IndexedDB, allow export only through an explicit warning/reveal action, and never upload or log secret material. Do not add private-key/seed import. Phantom, Solflare, Backpack and other Wallet Standard wallets remain supported, and WalletConnect/Reown requires a new Sunder-scoped project ID.
- Sniper is P0. Build the real execution boundary and tests before claiming it is operational.
- Never store private keys in plaintext, transmit them, log them, or commit them. Browser Wallet Standard and encrypted device-local embedded signing are self-custody boundaries. A persistent executor must use a separately configured encrypted signer policy and never reuse the browser vault.
