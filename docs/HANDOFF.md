...
### 2026-08-03 funded Solana Mainnet acceptance correction

Those executor gates were subsequently provisioned for one dedicated bounded operator wallet. A real Pump Token-2022 exact-mint buy requested `0.001 SOL`, passed quote/build/simulation and the isolated signer policy, was accepted by standard RPC and Jito, and finalized canonically at signature `2ky1Hz1GHFLCFN7msfjDLjBLH5gFn7BobGr2TzdN9GoBZLcs8N4YFf22TnPiNXq21SwkKRMAmvMU4AojXeFf2BK9`. RPC verified `err: null` and `35,323.892686` output tokens. The first tracker observation was falsely `expired` because the provider returned a slot-like `getBlockHeight`; confirmation now uses `isBlockhashValid` plus a final signature lookup, and the append-only audit was reconciled so risk hydration counts the confirmed spend. No second transaction was sent. This verifies the funded execution pipeline, not catch-all unattended trading or an automatic match by the still-narrow keyword rule. Full evidence is in `docs/evidence/2026-08-03-solana-mainnet-sniper-acceptance.md`.
