...
export function SettingsScreen() {
  ...
  return <div className="screen"><ScreenHeading eyebrow="Non-secret preferences" title="Settings" description="Chain-specific transaction defaults and truthful infrastructure readiness." /><Tabs.Root defaultValue="general" className="settings-tabs"><Tabs.List className="settings-tabs__list"><Tabs.Trigger value="general">General</Tabs.Trigger><Tabs.Trigger value="swap">Swap defaults</Tabs.Trigger><Tabs.Trigger value="relay">RPC & relays</Tabs.Trigger><Tabs.Trigger value="security">Security</Tabs.Trigger></Tabs.List><Tabs.Content value="general"><Panel title="Interface"><Toggle checked={notifications} onCheckedChange={setNotifications} label="Transaction notifications" description="Only provider/audit state, never invented confirmations." /><Toggle checked={compact} onCheckedChange={setCompact} label="Compact console density" description="Optimized for technical workflows." /><Field label="Activity filter"><Select defaultValue="confirmed"><option value="all">All states</option><option value="confirmed">Confirmed + failed</option><option value="execution">Execution only</option></Select></Field></Panel></Tabs.Content><Tabs.Content value="swap"><Panel title={`${family === "solana" ? "Solana" : "EVM"} defaults`}><div className="field-row"><Field label="Buy slippage %"><Input defaultValue="1.0" /></Field><Field label="Sell slippage %"><Input defaultValue="1.5" /></Field></div>{family === "solana" ? <div className="field-row"><Field label="Sniper tip SOL"><Input defaultValue="0.001010" /></Field><Field label="Swap tip SOL"><Input defaultValue="0.000100" /></Field></div> : <div className="field-row"><Field label="Priority fee Gwei"><Input defaultValue="1.5" /></Field><Field label="Replacement bump BPS"><Input defaultValue="1250" /></Field></div>}<div className="quick-actions-settings"><span>Quick buy</span>{["0.1", "0.25", "0.5", "1"].map((value) => <Badge key={value}>{value} {chain.nativeSymbol}</Badge>)}<span>Quick sell</span>{["25%", "50%", "75%", "100%"].map((value) => <Badge key={value}>{value}</Badge>)}</div></Panel></Tabs.Content><Tabs.Content value="relay"><Panel title="Provider readiness"><div className="readiness-table"><div><span>Standard RPC</span><Badge tone="good">Public fallback</Badge></div><div><span>{family === "solana" ? "Jito" : "Flashbots Protect"}</span><Badge tone={family === "evm" ? "good" : "warn"}>{family === "evm" ? "Official endpoint" : "Credential required"}</Badge></div><div><span>{family === "solana" ? "Nozomi / 0slot" : "Private transaction auth"}</span><Badge tone="warn">Server config required</Badge></div><div><span>Persistent executor</span><Badge tone="warn">Not on Vercel</Badge></div></div>{family === "evm" ? <Toggle checked={protect} onCheckedChange={setProtect} label="Prefer Flashbots Protect" description="Provider acceptance still requires canonical receipt confirmation." /> : null}</Panel></Tabs.Content><Tabs.Content value="security"><Panel title="Mainnet activation gates"><div className="gate-list"><span><Check size={15} /> Typed RPC configuration</span><span><Check size={15} /> Risk limits and kill switch</span><span><Lock size={15} /> External signer socket</span><span><Lock size={15} /> Relay/auth configuration</span><span><Lock size={15} /> Funding verification</span><span><Lock size={15} /> Operator confirmation</span></div><div className="custody-banner"><KeyRound size={19} /><div><strong>No private-key fields</strong><span>Browser UI accepts only wallet-provider signatures and public addresses.</span></div></div></Panel></Tabs.Content></Tabs.Root></div>;
}

export function DocsScreen() {
  const { family } = useNetwork();
  return <div className="screen docs-screen"><ScreenHeading eyebrow="Product runbook" title="Docs" description="Architecture, execution invariants, safe operating boundaries and network readiness." /><div className="docs-layout"><aside className="docs-toc"><a href="#architecture">Architecture</a><a href="#networks">Networks</a><a href="#wallets">Wallets</a><a href="#confirmation">Confirmation</a><a href="#readiness">Readiness</a><a href="#boundaries">Boundaries</a></aside><article className="docs-content"><section id="architecture"><h2>Chain-agnostic Sniper Engine</h2><p>The low-latency process normalizes events and runs a shared deterministic risk/retry/audit core. Chain adapters own quotes, transaction construction, relays, signing and confirmation semantics.</p><div className="docs-pipeline">EventSource <ChevronRight /> RuleEvaluator <ChevronRight /> QuoteAdapter <ChevronRight /> TransactionAdapter <ChevronRight /> Simulator <ChevronRight /> WalletAdapter <ChevronRight /> RelayRouter <ChevronRight /> ConfirmationAdapter</div></section><section id="networks"><h2>Production and verification networks</h2><div className="docs-grid"><Panel title="Solana"><p><strong>Mainnet:</strong> production mode, funded execution locked.</p><p><strong>Devnet:</strong> Wallet Standard self-transfer verification, Pump program boundary, standard RPC/Jito/Nozomi/0slot adapters.</p></Panel><Panel title="Ethereum / EVM"><p><strong>Mainnet:</strong> production mode, funded execution locked.</p><p><strong>Sepolia:</strong> EIP-1193 self-transfer verification, EIP-1559 simulation/replacement, receipt/reorg tracking, standard RPC/Flashbots adapters.</p></Panel></div></section><section id="wallets"><h2>Self-custody</h2><p>The web console never accepts a private key or seed phrase. Interactive {family === "solana" ? "Wallet Standard" : "EIP-1193"} signing stays in the browser wallet. Persistent automation uses a separate policy-limited signer over a Unix socket.</p></section><section id="confirmation"><h2>No success before chain confirmation</h2><p>A relay can return HTTP 200, an RPC can return a transaction hash, and a factory can predict an address while the transaction still fails to land. Sunder records those as submitted only. Success requires a canonical Solana signature status or EVM receipt at the configured depth; reorged, expired and reverted states fail.</p></section><section id="readiness"><h2>Production readiness matrix</h2><div className="matrix"><div className="matrix__head"><span>Capability</span><span>Solana Mainnet</span><span>Ethereum Mainnet</span><span>Test networks</span></div>{[["UI + selectors", "Implemented", "Implemented", "Implemented"],["Browser wallet", "Wallet Standard", "EIP-1193 / wagmi", "Executable"],["Quote / transaction", "Pump boundary", "Uniswap V2/V3/V4 auto-routing", "Mock/RPC tested"],["Relays", "RPC/Jito/Nozomi/0slot", "RPC/Flashbots", "Config aware"],["Funded executor", "Locked", "Locked", "Signer required"],["On-chain verification", "Required", "Required", "Executable from wallet modal"]].map((row) => <div key={row[0]}>{row.map((value, index) => <span key={value}><Badge tone={value === "Locked" ? "warn" : index === 0 ? "neutral" : "good"}>{value}</Badge></span>)}</div>)}</div></section><section id="boundaries"><h2>Explicitly excluded</h2><p>No fake volume, candle painting, wash trading, mixer/evasion, aged-wallet marketplace, fabricated holders, or invented deploy history. Safe replacements are scheduled DCA, rebalancing, transparent distribution, deterministic strategy simulation, load testing and honest analytics.</p></section></article></div></div>;
}
